...
const gameFunctions = {
  createMap: function (scene, levelDefinition) {
    var map = {};
  ...
  renderHud : function (renderer, hudCamera, scene) {
  ...
  createPacman : function () {
    ...
    createGhost : function () {
        ...
        var modelPath = 'pac-man_ghost_blinky.glb'; 
        return function (scene, position) {
    ...
    wrapObject : function (object, map) {
...
const genericFunctions = {
  ...
  animationLoop: function (callback, requestFrameFunction) {
...
const main = function () {
  const keys = genericFunctions.createKeyState();
  const renderer = gameFunctions.createRenderer();
  const scene = gameFunctions.createScene();
  const map = gameFunctions.createMap(scene, gameConstants.LEVEL);
  let numDotsEaten = 0;
...
var moveGhost = function () {
  ...
  genericFunctions.animationLoop(function (delta, now) {
    update(delta, now);
    ...
    // Render HUD
    gameFunctions.renderHud(renderer, hudCamera, scene);
  });
};
